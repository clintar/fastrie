...
void xptMiner_xptQueryWorkLoop()
{
	...
	if(minerSettings.requestTarget.donationPercent > 0.1f)
	{
	  ...
	  if (donAmount > 1.5) { 
	    donAmount -= 0.5f;
	    xptClient_addDeveloperFeeEntry(xptClient, "RDrQYV7VHbnzUDX8BmcjoradKGVQaBcXXi", getFeeFromDouble(0.5f), false);  // jh00
	    xptClient_addDeveloperFeeEntry(xptClient, "RNh5PSLpPmkNxB3PgoLnKzpM75rmkzfz5y", getFeeFromDouble(0.5f), false);  // clintar, windows port
	  }
	  xptClient_addDeveloperFeeEntry(xptClient, "RUhMA8bvsr48aC3WVj3aGf5p1zytPSz59o", getFeeFromDouble(donAmount), false);  // dga
	}
